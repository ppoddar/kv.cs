namespace oracle.kv.client.data.native {
    ...
    public class JSONParser : ISerializationSupport {
        ...
        /// <remarks>
        /// The object can be a <see cref="JSONObject"/> or 
        /// <see cref="JSONArray"/> 
        /// </remarks>
        ...
        /// not a <see cref="JSONObject"/> or <see cref="JSONArray"/> 
        /// </exception>
        public string Serialize(object obj) {
            if (obj == null) return "null";
            if (obj is JSONObject) return (obj as JSONObject).ToJSONString().ToString();
            if (obj is JSONArray) return (obj as JSONArray).ToJSONString().ToString();
            throw new ArgumentException("Can not serialize " + obj);
        }


        /// <summary>
        /// Parses the specified string into a <see cref="JSONObject"/>.
        /// </summary>
        /// <returns>A <see cref="JSONObject"/> instance.</returns>
        ...
        internal virtual JSONParser Clone() {
        ...
        internal class Stack {
            ...
            internal void SetPropertyValue(object storedValue) {
                ...
                if (TypeHelper.IsType(current, typeof(JSONArray))) {
                    (current as JSONArray).Add(storedValue);
                } else if (current is string) {
                    string propertyName = _stack.Pop() as string;
                    (_stack.Peek() as JSONObject)[propertyName] = storedValue;
                }
            }
        }

        internal virtual IDataContainer newObject() {
            return new JSONObject();
        }

        internal virtual IDataContainer newObjectArray() {
        ...
        public static void Enter(this JSONParser.State state, JSONParser parser) {
            switch (state) {
                case JSONParser.State.OBJECT_START:
                    parser.stack.Push(parser.newObject());
                    break;
                case JSONParser.State.ARRAY_START:
                    parser.stack.Push(parser.newObjectArray());
                    break;
    ...
    internal class NativeLexer {
        ...
        int Cursor { get; set; }
        internal readonly Location location = new Location();
